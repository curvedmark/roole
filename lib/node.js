...
Node.toListNode = function(rangeNode) {
	...
	while (
		...
		if (i++)
			itemNodes.push(new Node('separator', [' '], {loc: rangeNode.loc}));
	...
	if (!itemNodes.length)
		return new Node('null', {loc: rangeNode.loc});

	if (itemNodes.length === 1)
		return itemNodes[0];

	return new Node('list', itemNodes, {loc: rangeNode.loc});
};