...
logicalOr
	= first:logicalAnd rest:(_ 'or'i _ e:logicalAnd {return e;})* {
		...
		rest.forEach(function(operand) {
			node = new N('logical', [node, 'or', operand]);
		});
		return node;
	}
...
logicalAnd
	= first:equality rest:(_ 'and'i _ e:equality {return e;})* {
		var node = first;
		rest.forEach(function(operand) {
			node = new N('logical', [node, 'and', operand]);
		});
		return node;
	}
...
equality
	= first:relational rest:((_ o:('isnt'i / 'is'i) _ {return o;}) relational)* {
		...
		rest.forEach(function(array) {
			...
			var operand = array[1];
			node = new N('equality', [node, operator, operand]);
		});
		return node;
	}
...
relational
	= first:range rest:((_ o:$([<>]'='?) _ {return o;}) range)* {
		...
		rest.forEach(function(array) {
			var operator = array[0];
			var operand = array[1];
			node = new N('relational', [node, operator, operand]);
		});
		return node;
	}
...
additive
	= first:multiplicative rest:((_ c:[-+] s {return c;} / [-+]) multiplicative)* {
		...
		rest.forEach(function(array) {
			var operator = array[0];
			var operand = array[1];
			node = new N('arithmetic', [node, operator, operand]);
		})
		return node;
	}
...
unary
	...
	/ operator:[-+] operand:postfix {
		return new N('unary', [operator, operand]);
	}

postfix
	= value:primary posts:(argumentList / accessor)* {
		var node = value;
		posts.forEach(function(post) {
			var type = post.type === 'argumentList' ? 'call' : 'member';
			node = new N(type, [node, post]);
		})
		return node;
	}
...
assignment
	= variable:variable accessor:accessor? _ operator:$([-+*/?]? '=') _ value:list _ semicolon {
		var node = variable;
		if (accessor) { node = new N('member', [node, accessor]); }
		return new N('assignment', [node, operator, value]);
	}
