...
ruleList
	...
	/ (isodent / _) '{' indent first:rule rest:(isodent r:rule {return r})* solidOutdent '}' {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('ruleList', rest)
	}
...
propertyList
	= indent first:properties rest:(isodent p:properties {return p})* outdent {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('propertyList', rest)
	}
	/ (isodent / _) '{' indent first:properties rest:(isodent p:properties {return p})* solidOutdent '}' {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('propertyList', rest)
	}
	/ (isodent / _) '{' _ first:properties rest:(isodent p:properties {return p})* _ '}' {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('propertyList', rest)
	}
