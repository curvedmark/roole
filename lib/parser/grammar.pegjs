...
pseudoSelector
	= ':' doubled:':'? name:identifier argument:pseudoArguments? {
		return N('pseudoSelector', [name, argument || null], {doubled: !!doubled});
	}
...
assignment
	= variable:variable accessor:accessor? _ operator:$([-+*/?]? '=') _ value:list _ semicolon {
		var node = variable;
		if (accessor) { node = N('member', [node, accessor]); }
		return N('assignment', [node, operator, value]);
	}
...
propertyList
	= '{' propertyRules:(_ p:propertyRules {return p})? _ '}' {
		return N('propertyList', propertyRules || []);
	}
