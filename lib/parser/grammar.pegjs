...
property
	= star:'*'? name:identifier _ ':' _ value:list _ priority:'!important'? _ semicolon {
		...
		return Node('property', [name, value], {priority: priority || '', loc: loc()});
	}
...
mediaType
	= modifier:(m:('only'i / 'not'i) _ {return m;})? value:identifier {
		return Node('mediaType', [value], {modifier: modifier, loc: loc()});
	}
...
keyframes
	= '@' prefix:('-' p:$([a-z_]i [a-z0-9_]i*) '-' {return p;})? 'keyframes'i _ name:identifier _ keyframeList:keyframeList {
		return Node('keyframes', [name, keyframeList], {prefix: prefix || '', loc: loc()});
	}

keyframeList
	= '{' _ keyframeRules:keyframeRules? _ '}' {
		return Node('ruleList', keyframeRules || [], {loc: loc()});
	}
