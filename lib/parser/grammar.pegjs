...
rootRules
	= first:rootRule rest:((isodent / _) r:rootRule {return r})* {
		rest.unshift(first)
		return rest
	}
...
assignment
	= name:variable _ operator:$([-+*/?]? '=') _ value:(mixin / list) semicolon {
...
extend
	= '@extend'i s selectorList:singleLineSelectorList semicolon {
...
import
	= '@import'i s value:(string / url / variable) mediaQueryList:(_ m:mediaQueryList {return m})? semicolon {
...
if
	= '@if'i s condition:list consequence:ruleList alternative:((isodent / _) e:(elseIf / else) {return e})? {
		return N('if', [condition, consequence, alternative || null])
	}

elseIf
	= '@else if'i s condition:list consequence:ruleList alternative:((isodent / _) e:(elseIf / else) {return e})? {
		return N('if', [condition, consequence, alternative || null])
	}
...
propertyList
	...
	/ (isodent / _) '{' _ properties:properties _ '}' {
		return N('propertyList', properties)
	}
