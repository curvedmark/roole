...
ruleList
	= indent first:rule rest:(isodent r:rule {return r})* outdent {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('ruleList', rest)
	}
	/ (isodent / _) '{' indent first:rule rest:(isodent r:rule {return r})* outdent dents '}' {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('ruleList', rest)
	}
	/ (isodent / _) '{' _ first:rule rest:(isodent r:rule {return r})* _ '}' {
		rest.unshift(first)
		rest = _.flatten(rest)
		return N('ruleList', rest)
	}
...
property
	= star:'*'? name:identifier _ ':' _ propertyValue:propertyValue {
		if (star) {
...
propertyValue
	= value:expression _ priority:'!important'? {
...
if
	= '@if'i s condition:expression consequence:ruleList alternative:(isodent e:(elseIf / else) {return e})? {
		return N('if', [condition, consequence, alternative || null])
	}

elseIf
	= '@else if'i s condition:expression consequence:ruleList alternative:(isodent e:(elseIf / else) {return e})? {
		return N('if', [condition, consequence, alternative || null])
	}
...
for
	= '@for'i s value:variable index:(_ ',' _ i:variable {return i})? step:(s 'by'i s a:additiveExpression {return a})? s 'in'i s list:expression ruleList:ruleList {
