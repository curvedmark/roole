	...
	selectorNode.children.forEach(function(childNode, i) {
		...
		case 'ampersandSelector':
			if (!this.parentSelector)
				throw new Err("& selector is not allowed at the top level", childNode, this.filePath);
		...
		case 'combinator':
			if (!i) {
				if (!this.parentSelector)
					throw new Err("selector starting with a combinator is not allowed at the top level", childNode, this.filePath);
