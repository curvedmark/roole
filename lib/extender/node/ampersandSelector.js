...
Extender.prototype.visitAmpersandSelector = function(ampersandSelectorNode) {
	if (!this.parentSelector) {
		throw Err("& selector is not allowed at the top level", ampersandSelectorNode, this.filePath);
	}
	...
	if (valueNode) {
		...
		case 'typeSelector':
			break;
		default:
			throw Err("parent selector '" + lastNode.type + "' is not allowed to be appended", ampersandSelectorNode, this.filePath);
		}
