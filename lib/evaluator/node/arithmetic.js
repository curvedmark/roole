...
Evaluator.prototype.visitArithmetic = function(arithmeticNode) {
	...
	case 'dimension / percentage':
		...
		if (!divisor) {
			throw new Err('divide by zero', rightNode, this.filePath);
		}

		leftClone = Node.clone(leftNode);
	...
	case 'number / percentage':
		...
		rightClone = Node.clone(rightNode);
		rightClone.children[0] = leftNode.children[0] / divisor;
		return rightClone;
	}

	throw new Err("unsupported binary operation: '" + leftNode.type + "' " + operator + " '" + rightNode.type + "'", leftNode, this.filePath);
};