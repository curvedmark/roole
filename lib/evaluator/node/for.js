...
Evaluator.prototype.visitFor = function(forNode) {
	...
	if (stepNode) {
		...
		if (stepNumber === null) {
			throw new Err("step number must be a numberic value", stepNode, this.filePath);
		}

		if (!stepNumber) {
			throw new Err("step number is not allowed to be zero", stepNode, this.filePath);
		}
	}
	...
	if (listNode.type === 'null') {
		...
		if (IndexVariableNode) {
			...
			var IndexNode = new Node('null', {loc: IndexVariableNode.loc});
	...
	if (listNode.type !== 'list') {
		...
		if (IndexVariableNode) {
			var IndexVariableName = IndexVariableNode.children[0];
			var IndexNode = new Node('number', [0], {loc: IndexVariableNode.loc});
			this.scope.define(IndexVariableName, IndexNode);
		}
	...
	if (stepNumber > 0) {
		for (var i = 0, j = i, length = itemNodes.length; i < length; i += 2 * stepNumber, ++j) {
			iterate(itemNodes[i], j, i === length - 1);
		}
	} else {
		for (var i = itemNodes.length - 1, j = Math.floor(i / 2); i >= 0; i += 2 * stepNumber, --j) {
			iterate(itemNodes[i], j, !i);
		}
	}

	function iterate(itemNode, i, isLast) {
		that.scope.define(valueVariableName, itemNode);

		if (IndexVariableNode) {
			var IndexVariableName = IndexVariableNode.children[0];
			var IndexNode = new Node('number', [i], {loc: IndexVariableNode.loc});
			that.scope.define(IndexVariableName, IndexNode);
		}

		var ruleListClone = isLast ? ruleListNode : Node.clone(ruleListNode);
		that.visit(ruleListClone);
