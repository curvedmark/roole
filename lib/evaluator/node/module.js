...
Evaluator.prototype.visitModule = function(moduleNode) {
	...
	if (name === null) {
		throw Err("'" + nameNode.type + "' can not be used as a module name" , nameNode, this.filePath);
	}

	var separatorNode = this.visit(moduleNode.children[1]);
	var separator = separatorNode ? Node.toString(separatorNode) : '-';
	if (separator === null) {
		throw Err("'" + separatorNode.type + "' can not be used as a module name separator" , separatorNode, this.filePath);
	}
