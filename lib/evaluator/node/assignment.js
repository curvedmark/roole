...
Evaluator.prototype.visitAssignment = function(assignmentNode) {
	...
	if (targetNode.type === 'null') {
		throw Err("can not assign to items in 'null'", targetNode, this.filePath);
	}
	...
	if (range === null) {
		throw Err("'" + accessorNode.type + "' can not be used to access items in a list", accessorNode, this.filePath);
	}
	...
	if (range.to <= 0) {
		...
		if (itemNodes.length > 1) {
			...
			separatorNode = Node('separator', [' '], {loc: valueNode.loc});
		}

		var offset = -range.to;
		if (offset) {
			var nullNode = Node('null', {loc: valueNode.loc});
	...
	var node = itemNodes.length === 1 ?
		itemNodes[0]:
		Node('list', itemNodes, {loc: valueNode.loc});
